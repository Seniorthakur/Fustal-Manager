...
export async function updateCourtStatusAction(formData: FormData): Promise<void> {
  const user = await requirePermission("courts:manage");
...
export async function deletePricingRuleAction(formData: FormData): Promise<void> {
  const user = await requirePermission("pricing:manage");
  const id = String(formData.get("id") ?? "");
